...
###Creating the timer
 ...
 The interval is how often, in milliseconds, you want the timer to change, or any other functions to fire.  
   ...
   ```
There is a global timers[] array which we use to access the timer(s) when the window calls the setInterval function inside the timer. When you pass in a unique id it sets that id as the index of that timer in the timers[] array
